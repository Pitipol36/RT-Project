...
class Integrator():
    ...
    def compute_scattering(self, rGen_ray, scene, maxDepth):
        ...
        # --- FAKE BLACK HOLE LENSING ---
        bh_center = rtu.Vec3(0, 0, 5)  
        bh_radius = 0.5                 
        bh_mass = 0.7 
        
        ray_orig = rGen_ray.getOrigin()
        ray_dir = rtu.Vec3.unit_vector(rGen_ray.getDirection())
        
        oc = bh_center - ray_orig
        t_closest = rtu.Vec3.dot_product(oc, ray_dir)
        
        # 1. Fire a straight ray FIRST
        found_hit = scene.find_intersection(rGen_ray, rtu.Interval(0.000001, rtu.infinity_number))
        
        # 2. Did we hit a foreground object? 
        is_foreground = False
        if found_hit and (scene.getHitList().getT() < t_closest):
            is_foreground = True
            
        # 3. Only bend if the ray safely reaches the black hole without hitting the foreground
        if not is_foreground and (t_closest > 0):
            closest_point = ray_orig + (ray_dir * t_closest)
            dist_to_center = (closest_point - bh_center).len()
            
            if dist_to_center < bh_radius * 5.0:
                deflection_strength = bh_mass / (dist_to_center * dist_to_center)
                pull_dir = rtu.Vec3.unit_vector(bh_center - closest_point)
                
                bent_dir = rtu.Vec3.unit_vector(ray_dir + (pull_dir * deflection_strength))
                
                rGen_ray = rtr.Ray(closest_point, bent_dir, rGen_ray.getTime())
                
                # Re-shoot the bent ray to see what the gravity reveals
        found_hit = scene.find_intersection(rGen_ray, rtu.Interval(0.000001, rtu.infinity_number))
        if found_hit == True:
            # get the hit info
